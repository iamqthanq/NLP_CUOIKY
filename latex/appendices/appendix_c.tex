...
\begin{itemize}
    \item \textbf{Baseline Model Checkpoint}: \texttt{vanilla\_best\_model.pth} \\
    \textit{Size}: ~50 MB \\
    \textit{Contains}: Vanilla model state dict, optimizer state, training metrics \\
    \textit{Epoch}: 13 (early stopping) \\
    \textit{Validation Loss}: 4.139 \\
    \textit{BLEU Score}: 29.12\% \\
    \url{https://drive.google.com/file/d/<YOUR_FILE_ID>/view?usp=sharing}
    
    \item \textbf{Attention Model Checkpoint}: \texttt{attention\_best\_model.pth} \\
    \textit{Size}: ~120 MB \\
    \textit{Contains}: Attention model state dict, optimizer state, training metrics \\
    \textit{Epoch}: 16 (early stopping) \\
    \textit{Validation Loss}: 3.789 \\
    \textit{BLEU Score}: 36.57\% \\
    \url{https://drive.google.com/file/d/<YOUR_FILE_ID>/view?usp=sharing}
