...
\begin{itemize}
    ...
    \item Early stopping kích hoạt tại epoch 17 (patience=3, best epoch 13)
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Training log (Vanilla, chạy trên GPU) - trích các mốc chính}
\begin{tabular}{@{}rrrrrl@{}}
\toprule
\textbf{Epoch} & \textbf{Train Loss} & \textbf{Val Loss} & \textbf{Train PPL} & \textbf{Val PPL} & \textbf{Note} \\
\midrule
1  & 4.906 & 4.777 & 135.081 & 118.761 & Start \\
5  & 3.052 & 3.729 & 21.150  & 41.638  & Improving \\
10 & 2.069 & 3.268 & 7.918   & 26.252  & Stable \\
13 & 1.701 & 3.161 & 5.481   & 23.594  & \textbf{Best val loss} \\
15 & 1.502 & 3.260 & 4.489   & 26.058  & End (no early-stop) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title=Model With Attention + Greedy]
\begin{center}
\textbf{BLEU Score: 36.24\%}

Corpus size: 200 câu test (sampled) \\
Decoding: Greedy (argmax) \\
Improvement: \textbf{+7.45\% so với Vanilla}
\end{center}
\end{tcolorbox}

\begin{tcolorbox}[colback=orange!5!white, colframe=orange!75!black, title=Model With Attention + Beam Search (Extension đầy đủ)]
\begin{center}
\textbf{BLEU Score: 38.04\%}

Corpus size: 200 câu test (sampled) \\
Decoding: Beam Search (K=5) \\
Improvement: \textbf{+5.21\% so với Vanilla} \\
\textit{Đây là cấu hình Extension theo đúng yêu cầu đề bài}
\end{center}
\end{tcolorbox}

\textbf{Đánh giá:}
\begin{itemize}
    \item [\checkmark] Vanilla + Greedy (Baseline): 32.83\% - Đạt yêu cầu đề bài ($\geq$ 20\%)
    \item [\checkmark] Attention + Beam Search: 38.04\% - Extension đầy đủ, +5.21\% so với Baseline
    \item [!] Vẫn thấp hơn SOTA (Transformer): ~42\% $\rightarrow$ Gap ~4\%
\end{itemize}
...
\subsection{Ví dụ 4: Lỗi từ hiếm (BLEU = 22.5\%)}
...
\begin{itemize}
    \item \textbf{Hạn chế:} Vocab 10K có hạn chế với từ chuyên ngành như "motorcyclist"
    ...
    \begin{enumerate}
        \item Tăng vocab size: 10K $\rightarrow$ 15K cho từ hiếm hơn (xem Extension)
...
Các hạn chế chính:
\begin{itemize}
    \item Vocab 10K hạn chế với từ chuyên ngành
    \item Context vector cố định không đủ cho câu dài
    \item Chưa xử lý được cấu trúc phức tạp (adjective placement)
    \item Mất thông tin khi câu nguồn >15 từ
\end{itemize}

\textbf{Chương 5} sẽ trình bày mô hình Extension với Attention Mechanism để khắc phục các hạn chế trên.
