...
\midrule
Cài đặt Encoder-Decoder & 3 layers, 1024 hidden & 3.0/3.0 \\
Xử lý dữ liệu (DataLoader) & Pack/pad + sort & 2.0/2.0 \\
Training + Early stopping & 20 epochs, patience=5 & 1.5/1.5 \\
Hàm translate() & Greedy + Beam(K=5) & 1.0/1.0 \\
...
\textbf{Mô tả:} Thay thế LSTM bằng Transformer với self-attention và multi-head attention.

\textbf{Ưu điểm:}
\begin{itemize}
    \item Parallelizable training → nhanh hơn 5-10x
    \item Self-attention: Nắm bắt phụ thuộc xa tốt hơn LSTM
    \item Multi-head attention: Học nhiều patterns đồng thời
    \item Positional encoding thay vì recurrent
\end{itemize}

\textbf{Lợi ích:}
\begin{itemize}
    \item Giảm thời gian training từ 2.4h → 30 phút
    \item Cải thiện BLEU với câu dài (>15 từ)
    \item State-of-the-art architecture
\end{itemize}

\textbf{Ước tính:} BLEU 36.57\% → 42-46\% (với Transformer)
...
\textbf{Ước tính:} BLEU 36.57\% → 38-40\%
...
\midrule
\textbf{Mục tiêu cuối} & & \textbf{~1 tháng} & \textbf{$\geq$ 40\%} \\
\bottomrule
\end{tabular}
\end{table}
