...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Baseline}: BLEU 29.12\% (vượt yêu cầu 20\% đến 45\%)
        \item \textbf{Extension}: BLEU 36.57\% (vượt xa yêu cầu, đạt mức tốt)
        \item \textbf{Cải thiện}: +7.45\% (tuyệt đối), +25.6\% (tương đối)
        \item Training time: 1.0h (Baseline) + 2.4h (Extension) = 3.4h tổng
    \end{itemize}
    
    \item \textbf{So sánh chi tiết Vanilla vs Attention}:
    \begin{itemize}
        \item Phân tích BLEU theo độ dài câu: Attention vượt trội với câu dài (+7.52\%)
        \item Phân tích theo loại lỗi: Attention giảm lỗi câu dài từ 38\% → 18\%
        \item 2 ví dụ minh họa cụ thể cách Attention giải quyết vấn đề
...
\textbf{Ước tính kết quả:} BLEU 36.57\% → 42-46\% (tăng +5-10\%)
...
\textbf{Ước tính kết quả:} BLEU 36.57\% → 37-38\% (tăng +1-2\%)
...
\textbf{Ước tính kết quả:} BLEU 36.57\% → 39-41\% (tăng +3-5\%)
...
\textbf{Ước tính kết quả:} BLEU 36.57\% → 38-39\% (tăng +2-3\%)
