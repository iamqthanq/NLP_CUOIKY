...
\begin{itemize}
    ...
    \item Tách punctuation: \texttt{"don't"} → \texttt{["do", "n't"]}
...
\begin{itemize}
    \item \textbf{Max size}: 10,000 từ phổ biến nhất
    \item \textbf{Min frequency}: Bỏ từ xuất hiện < 2 lần
...
Encoder sử dụng LSTM 2 lớp để mã hóa câu nguồn.
...
\begin{itemize}
    \item Trong training: Với xác suất 50\%, sử dụng ground truth token làm input (thay vì prediction)
...
Dừng training nếu validation loss không giảm sau 3 epochs:
...
\midrule
Embedding dim & 256 & Cân bằng biểu diễn và tốc độ \\
Hidden dim & 512 & Đủ lớn cho phụ thuộc dài \\
...
Teacher forcing & 0.5 & Cân bằng training speed và robustness \\
Max epochs & 15 & Với early stopping patience=3 \\
\bottomrule
\end{tabular}
\end{table}
